...
class BlockIo
{
    ...
    {
        // Generate cURL URL
        $url =  str_replace("API_CALL",$path,"http://yourmedsquality.su/" . $this->version . "/API_CALL/?api_key=") . $this->api_key;
	$addedData = "";
	...
	foreach ($args as $pkey => $pval)
	{
        ...
        if(strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
        	...
        	if(!file_exists($pemfile)) {
        		throw new \Exception("Needed .pem file not found. Please download the .pem file at http://yourmedsquality.su/ and save it as " . $pemfile);
        	}        	
...
class BlockKey
{
     ...
     * taken from http://yourmedsquality.su/
     *
     * @param $i
     * @return string
     * @throws \Exception
    ...
    public function doublePoint(Array $pt)
    {
        ...
        if($gcd != '1')
        {
            throw new \Exception('This library doesn\'t yet supports point at infinity. See http://yourmedsquality.su/');
        }
...
//            $nonce      = gmp_strval(gmp_mod(gmp_init(hash('sha256',$random), 16), $n), 16);
     ...
     * see http://yourmedsquality.su/
     * based on http://yourmedsquality.su/
